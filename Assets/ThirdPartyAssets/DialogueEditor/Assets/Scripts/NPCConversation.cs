...
namespace DialogueEditor
{
    ...
    public class NPCConversation : MonoBehaviour
    {
        ...
        // Improving the GetNodeData method to handle GameObject creation and component addition more safely.
        public NodeEventHolder GetNodeData(int id)
        {
            // Ensure NodeSerializedDataList is initialized
            NodeSerializedDataList ??= new List<NodeEventHolder>();

            // Try to find existing NodeEventHolder
            var nodeHolder = NodeSerializedDataList.Find(n => n.NodeID == id);
            if (nodeHolder != null) return nodeHolder;

            // Create a new NodeEventHolder if not found
            Transform eventInfo = transform.Find(CHILD_NAME) ?? new GameObject(CHILD_NAME).transform;
            eventInfo.SetParent(transform, false);

            NodeEventHolder newHolder = eventInfo.gameObject.AddComponent<NodeEventHolder>();
            newHolder.NodeID = id;
            newHolder.Event = new UnityEngine.Events.UnityEvent();
            NodeSerializedDataList.Add(newHolder);
            return newHolder;
        }
        ...
        public void CopyParamActions(EditableConversationNode editable, ConversationNode node)
        {
            ...
            for (int i = 0; i < editable.ParamActions.Count; i++)
            {
                if (editable.ParamActions[i].ParamActionType == EditableSetParamAction.eParamActionType.Int)
                {
                    ...
                    {
                        ParameterName = setIntEditable.ParameterName,
                        Value = setIntEditable.Value
                    };
                ...
                else if (editable.ParamActions[i].ParamActionType == EditableSetParamAction.eParamActionType.Bool)
                {
                    ...
                    {
                        ParameterName = setBoolEditable.ParameterName,
                        Value = setBoolEditable.Value
                    };
        ...
        private void CopyConnectionConditions(EditableConnection editableConnection, Connection connection)
        {
            ...
            for (int i = 0; i < editableConditions.Count; i++)
            {
                if (editableConditions[i].ConditionType == EditableCondition.eConditionType.BoolCondition)
                {
                    ...
                    {
                        ParameterName = ebc.ParameterName
                    };
                ...
                else if (editableConditions[i].ConditionType == EditableCondition.eConditionType.IntCondition)
                {
                    ...
                    {
                        ParameterName = eic.ParameterName
                    };
