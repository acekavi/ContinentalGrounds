...
namespace DialogueEditor
{
    ...
    public class Conversation
    {
        ...
        public void SetInt(string paramName, int value, out eParamStatus status)
        {
            ...
            if (param != null)
            {
                Debug.Log($"Setting Int Parameter: {paramName} to {value}");
                param.IntValue = value;
                status = eParamStatus.OK;
            }
            else
            {
                Debug.LogError($"Failed to find Int Parameter: {paramName}");
        ...
        public void SetBool(string paramName, bool value, out eParamStatus status)
        {
            ...
            if (param != null)
            {
                Debug.Log($"Setting Bool Parameter: {paramName} to {value}");
                param.BoolValue = value;
                status = eParamStatus.OK;
            }
            else
            {
                Debug.LogError($"Failed to find Bool Parameter: {paramName}");
                status = eParamStatus.NoParamFound;
            }
        }
